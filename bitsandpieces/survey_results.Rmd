...
```{r, results='asis', echo=FALSE, eval = !xremote}
cat("- this example uses locally stored data (and may be not up to date)\n")
```

```{r, results='asis', echo=FALSE, eval = xremote}
cat("- this example grabs the data directly from the survey website\n")
```
...
# write local table if needed
# write.csv(xdata, file = "data_files/survey_results.csv", row.names = FALSE, quote = FALSE)

```


```{r, echo=FALSE, fig.width=10, fig.height=5, out.width="90%", fig.retina=2, fig.align='center'}
...
barplot(table(xdata$tried), main = "did you try reproducing\nothers' results?", ylim = c(0, 20), xpd = TRUE)
```
